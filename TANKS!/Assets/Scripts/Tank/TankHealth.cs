...
public class TankHealth : MonoBehaviour
{
    ...
    private void OnDeath()
    {
        ...
        {
            GameObject newTank;
            newTank = Instantiate(TankPrefab, TankSpawn.position, TankSpawn.rotation);
            Debug.Log("SpawnTank");
        }
    ...
    void Start()
    {
        TankSpawn = GameObject.FindGameObjectWithTag("SpawnPoint").transform;
    }

    // Update is called once per frame
    void Update()
    {
        //test for tank respawn
        if (m_Dead == true)
